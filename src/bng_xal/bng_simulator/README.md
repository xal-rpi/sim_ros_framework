...
## Important Scripts (`bng_simulator/scripts/`)

While the primary interaction is through the `sim_manager_node` via the launch file, the following scripts provided in `bng_simulator/scripts` offer additional functionalities:

*   **`run_simulator.py`**: This script is the underlying Python entry point for the `sim_manager_node` executable. It can be run directly for debugging or special use cases.
*   **`sim_shell.py`**: An interactive command-line shell for direct interaction with the simulation, useful for debugging, manual control, or inspecting simulation state. Run with `ros2 run bng_simulator sim_shell`.
*   **`start_logs.py`**: A utility to start logging GtState sensor data to a pickle file. Run with `ros2 run bng_simulator start_logs`.
*   **`find_test_ema_gtstate.py`**: A utility to help tune Exponential Moving Average (EMA) parameters used in `gtstate.lua` (part of `luamod`).
