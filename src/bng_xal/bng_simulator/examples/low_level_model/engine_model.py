...
class MLP(nn.Module):
    ...
    def setup(self):
        ...
        self._activation_fn = get_activation_fn_from_name(self.activation_fn) \
            if isinstance(self.activation_fn, str) else self.activation_fn
...
class EngineModel(nn.Module):
    ...
    def normalize_state_action(self, data: Dict[str, jnp.ndarray]):
        ...
        return {
            name : (val - self.state_action_mean[name]) / self.state_action_std[name] \
                for name, val in data.items() if name in self.state_action_mean
        }
    ...
    def get_data_from_state_and_control(
        self, state: jnp.ndarray, control: jnp.ndarray
    ):
        """Get the data dictionary from the state and control.
        """
        res_unscaled = {name : state_val for name, state_val in zip(self.name_states, state)}
    ...
    def combine_state_and_latent(
        self, state: jnp.ndarray, latent_state: jnp.ndarray
    ):
    ...
    def calculate_latent_space_field(self, data):
        ...
        inv_gear_ratio_eq = self.gear_ratio_dyn(self.convert_features_to_nn_input(feat_gr, data))[0]
        tau_gear = jnp.exp(self.tau_gear)
        gear_ratio_dot = (inv_gear_ratio_eq - inv_gear_ratio) * tau_gear
        return jnp.array([gear_ratio_dot, we_u, we_r]), {"inv_gear_ratio_eq": inv_gear_ratio_eq}
    ...
    def calculate_initial_guess_for_latent(self, state, control, times):
        ...
        init_state = state[0]
        init_gear_ratio = init_state[2] / init_state[0] # Assume no zero speed and good index
    ...
    def initial_guess_wrapper(
        ...
        return self.apply(
            learnable_parameters, state, control, times,
...
def integrate_path(
    ...
    dt_array = jnp.repeat(dt_array / num_iter_between_dt, num_iter_between_dt) # shape: (T * num_iter_between_dt,)
    controls = jnp.repeat(controls, num_iter_between_dt, axis=0) # shape: (T * num_iter_between_dt,)
    ...
    def step_fn(carry, inputs):
        ...
        noise_term, noise_extra = noise.noise_wrapper(params["noise"], reduced_state, control)
    ...
    if history_data is not None:
        ...
        latent_initial_state = dynamics.initial_guess_wrapper(params["drift"], *history_data)
    ...
    if return_full_state:
        return jnp.concatenate([full_state[None], path], axis=0)[::num_iter_between_dt], extras
    return jnp.concatenate([init_state[None], path], axis=0)[::num_iter_between_dt], extras
...
def compute_nll_loss(
    ...
    )(states[0], controls, times, rng, params, history_data, dynamics, noise, False, num_iter_between_dt)
    # Extract noise term
...
def single_sequence_training_loss(
    ...
    loss_dict, (mse_state, pred_state), extras = compute_nll_loss(
        ...
        noise_term, discounts, extra_loss_args.get("num_iter_between_dt", 1),
...
def load_model_from_config(
    ...
    if "state_min" in data_statistics:
        drift_args["state_min"] = {
            **drift_args.get("state_min", {}), **data_statistics["state_min"]
        }
    if "state_max" in data_statistics:
        drift_args["state_max"] = {
            **drift_args.get("state_max", {}), **data_statistics["state_max"]
        }
...
class ModelWithTargetWheelSpeedAndRate:
    ...
    @property
    def name_controls(self) -> Tuple[str]:
        return [f"{name}_dot" for name in self.model.name_controls] + ["forced_road_torque",]
    @property
    def num_controls(self) -> int:
        return len(self.name_controls)
    @property
    ...
    def bounds_latents(self) -> Tuple[Dict[str, float]]:
        ...
        min_dict = {**min_dict, **{name : self.model.state_min[name] for name in self.model.name_controls}}
        max_dict = {**max_dict, **{name : self.model.state_max[name] for name in self.model.name_controls}}
    ...
    def drift_wrapper(self, params, state, control):
        ...
        base_state_dot, extra_base = self.model.drift_wrapper(params, base_state, base_control)
        ...
        extra_state_dot = {
            f"{_name}_dot": _val \
                for _name, _val in zip(self.model.name_states + self.name_latents, full_state_dot)
        }
...
class EngineModelEnv(environment.Environment):
    ...
    def reset_env(self, key: jnp.ndarray, params: EnvParams):
        ...
        latent_state = self.drift.initial_guess_wrapper(self.model_params["drift"],
            init_cond_states[idx], init_cond_controls[idx], init_times[idx]
        )
        # Combine the state and latent state
        full_state = self.drift.combine_state_and_latent(init_cond_states[idx][-1], latent_state)
    ...
    def predict_trajectories(
        ...
        state_evol, extra = integrate_path(
            state, _control, _times, key,
            self.model_params, None, self.drift, self.noise,
    ...
    def setup_target_config(
        ...
        Tsim = self.env_config["sampling_time"] \
                * self.env_config["max_steps_in_episode"]
    ...
    def get_obs_and_more(
        self, env_state: EnvState, extra_values: Dict[str, Any] = {}
    ):
    ...
    def setup_observation_space(self):
        ...
        state_min += [self.drift.bounds_latents[0][name] for name in self.drift.name_latents]
        state_max += [self.drift.bounds_latents[1][name] for name in self.drift.name_latents]
        ...
        self._obs_min = jnp.array([self._state_min.get(name, -MAX_VAL) for name in self._obs_names])
        self._obs_max = jnp.array([self._state_max.get(name, MAX_VAL) for name in self._obs_names])
        ...
        self._action_min = jnp.array([action_bounds[name][0] for name in self._action_names])
        self._action_max = jnp.array([action_bounds[name][1] for name in self._action_names])
    ...
    def action_space(self, params= None):
        ...
        return spaces.Box(
            ...
            self._action_min.shape, jnp.float32
        )
        ...
        return spaces.Dict(
            {
                "x": spaces.Box(
                    ...
                    self._state_min.shape, jnp.float32
                ),
                ...
                "des_wheel_speed": spaces.Box(
                    ...
                    self.drift.model.state_max["rear_wheelspeed"],(), jnp.float32
                ),
    ...
    def observation_space(self, params: EnvParams) -> spaces.Box:
        ...
        return spaces.Box(
            ...
            self._obs_min.shape, jnp.float32
        )
