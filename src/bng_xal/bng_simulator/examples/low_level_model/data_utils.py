...
def get_mean_and_scale_fields(
    ...
    fields_data = {
        key : np.concatenate(
            [ traj[key] for traj in dataset["trajectories"] ], axis=0
        ) for key in fields
    }
    fields_info_data = {
        key : np.concatenate(
    ...
    mean_fields = \
        { key : np.mean(value) for key, value in total_fields_data.items() }
    scale_fields = \
        { key : np.std(value) for key, value in total_fields_data.items() }
    # Let's take care of too small scales and set them to 1
    scale_fields = \
        { key : (scale if scale > 1e-10 else 1.0) \
            for key, scale in scale_fields.items()
        }
...
def get_min_max_fields(
    ...
    total_fields_data = {**fields_data, **fields_info_data}
    min_fields = \
        { key : np.min(value) for key, value in total_fields_data.items() }
    max_fields = \
        { key : np.max(value) for key, value in total_fields_data.items() }
...
def remove_irrelevant_fields(
    ...
    Returns:
        dataset: The dataset with the relevant fields
            dict
    """
    for traj, traj_info in \
        zip(dataset["trajectories"], dataset["trajectories_info"]):
...
def split_time_dependent_parameters(
    ...
    actual_time_dependent_parameters = [
        key for key in time_dependent_parameters \
    ...
    constant_parameters = [
        key for key in time_dependent_parameters \
            if key in system_physical_params
    ]
...
def convert_transitions_to_array(
    ...
    extra_parameters = {
        key : np.array([ [infos['system_physical_params'][key]] * horizon \
                for infos in transitions_info
        ]) for key in system_physical_params
    }
    return state_arr, control_arr, \
        {**time_dependent_parameters, **extra_parameters}
