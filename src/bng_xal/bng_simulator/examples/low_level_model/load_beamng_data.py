...
def find_consecutive_true(metrics, min_length=-1):
    ...
    section_inds_ = np.split(
        np.r_[: len(full_auto)], np.where(np.diff(full_auto) != 0)[0] + 1
    )
    full_auto_inds = []
...
def format_gtstate_traj(
    ...
    if len(valid_indexes) == 0:
        success = False
        tqdm.write(f"No subset of length < {min_traj_len} is invertible")
        tqdm.write("-----------------\n")
    ...
    for run in runs:
        ...
        for (vehicle_name, veh_sensor), curr_data in data.items():
            ...
            if veh_sensor != sensor_type or veh_model_type != vehicle_model:
                tqdm.write(
                    f"Ignoring {vehicle_name} - {veh_sensor} of type {veh_model_type} from {run_name}"
                )
                continue
        ...
        for info, curr_data in data_of_interest.items():
            ...
            (_trajectories, _trajectories_info), _success = format_gtstate_traj(
                metadata,
                curr_data,
                valid_trajectory_cond=valid_trajectory_cond,
                min_traj_len=min_traj_len,
                indx_traj=len(m_trajectories),
            )
            if not _success:
                continue
            num_success += 1
