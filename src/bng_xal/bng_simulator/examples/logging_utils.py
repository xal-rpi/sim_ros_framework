...
def dump_yaml(config_path: str, dict_val: Dict[str, Any]):
...
def convert_dict_of_array_to_dict_of_list(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in config.items():
        if isinstance(value, Mapping):
            new_config[key] = convert_dict_of_array_to_dict_of_list(value)
        elif isinstance(value, list):
            new_config[key] = [convert_dict_of_array_to_dict_of_list(v) for v in value]
        elif isinstance(value, tuple):
            new_config[key] = tuple(
                [convert_dict_of_array_to_dict_of_list(v) for v in value]
            )
        else:
            new_config[key] = value.tolist() if hasattr(value, "tolist") else value
    return new_config


def load_saved_data_from_checkpoint(
    experiment_dir: str, checkpoint_name: str, best_mode: str = "min", step: int = -1
) -> Dict[str, Any]:
    """
    ...
    ckpt_manager = TrainCheckpoints(
        experiment_dir, checkpoint_name, {}, best_mode=best_mode, writer_on=False
    )
    # Restore the data
...
class TrainCheckpoints:
    ...
    def __init__(
        ...
        if writer_on:
            ...
            if len(extra_config_to_save_as_yaml) > 0:
                ...
                extra_config_to_save_as_yaml = convert_dict_of_array_to_dict_of_list(
    ...
    def init_checkpoints_format(self):
        ...
        def best_fn(x):
            ...
            """
            value = 0.0
            inexistant_key_default = np.inf if self.best_mode == "min" else -np.inf
        ...
        if not self.async_exec:
            ...
            orbax_checkpointer = ckpts.AsyncCheckpointer(
                ckpts.PyTreeCheckpointHandler(), timeout_secs=self.timeout_async
            )
    ...
    def write_checkpoint(
        self, step: int, save_dict: Dict[str, Any], metrics_val: Dict[str, float]
    ):
        """
        ...
        self.checkpoint_manager.save(
            step, save_dict, save_kwargs={"save_args": save_args}, metrics=metrics_val
        )
