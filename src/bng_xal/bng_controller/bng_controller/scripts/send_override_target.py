...
class OverrideClientNode(Node):
    def __init__(self):
        ...
        self.client = self.create_client(
            OverrideTargets, "/high_level_controller/override_targets"
        )
        while not self.client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "Service /high_level_controller/override_targets not available, waiting again..."
            )
    ...
    def send_request(self, labels, values, lifetime):
        ...
        try:
            ...
            self.get_logger().error(
                f"Invalid input for target_values (must be numbers): {e}"
            )
            return None
        ...
        try:
            ...
            self.get_logger().error(
                f"Invalid input for lifetime_sec (must be a number): {e}"
            )
            return None
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Send override targets to the high_level_controller."
    )
    parser.add_argument(
        "--labels",
        nargs="+",
        required=True,
        help="List of target labels (e.g., road_wheel_angle engine_torque)",
    )
    parser.add_argument(
        "--values", nargs="+", required=True, help="List of corresponding target values"
    )
    parser.add_argument(
        "--lifetime",
        type=float,
        required=True,
        help="Lifetime of the override in seconds",
    )
    ...
    response = override_client_node.send_request(
        parsed_args.labels, parsed_args.values, parsed_args.lifetime
    )
