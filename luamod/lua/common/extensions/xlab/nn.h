...
int nn_set_dense_layer(Model *model, int idx, int in_features, int out_features,
...
float *nn_run(Model *model, const float *input, int input_size, int *out_size);

// free model and its weights
void nn_free_model(Model *model);

// free any buffer returned by nn_run
void nn_free_buffer(float *buffer);
